...
const UserDisplayAdTable = () => {
  ...
  const getAdData = async (req, res) => {
    const fetchResponse = await fetch(
      "http://localhost:1337/api/ads?populate=*"
    );
    const adJSON = await fetchResponse.json();
    const requestedAds = adJSON.data.filter(
      (value) => value.attributes.isApproved === true
    );
    ...
    <>
      {adData.length > 0 &&
        adData.map((curElem, index) => {
          link = `https://www.${curElem.attributes.Website}`;
          return (
                ...
                <div class="px-8">
                  ...
                  <div class="flex items-center justify-center space-x-10">
                    <div class="md:ml-[6px] flex flex-col items-center lg:flex-row  bg-white">
                      <img
                        // // src={`http://localhost:1337${curElem?.attributes?.Image?.data[0]?.attributes?.formats?.thumbnail?.url}`}
                        src=""
                        class=" "
                        alt=""
                      />
